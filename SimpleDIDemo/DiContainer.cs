...
namespace SimpleDIDemo
{
    public class DiContainer
    {
        ...
        private object GetImplementation(Type type)
        {
            ...
            var constructorParametersTypes = constructorInfo.GetParameters();

            List<object> constructorParametersImplementations = new List<object>();

            foreach (var param in constructorParametersTypes)
            {
                constructorParametersImplementations.Add(GetImplementation(param.ParameterType));
            }

            return constructorInfo.Invoke(constructorParametersImplementations.ToArray());
        }
    }
}
